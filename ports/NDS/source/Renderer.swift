...
//  into a 16bpp ARGB1555 row-major canvas (bit 15 set = opaque, the DS's
//  bitmap-background format); main.swift copies the canvas into the
//  16bpp bitmap background's VRAM.
//
//  The DS screens are 256x192, so the UI runs slightly smaller than the
//  real iPod Classic LCD: 7 menu rows instead of 9, same metrics.
//
...
let iPodWidth: Int32 = 256
let iPodHeight: Int32 = 192
