...
//  into a 32bpp ARGB8888 row-major canvas; AndroidMain.swift uploads the
//  canvas straight into an SDL streaming texture (SDL_PIXELFORMAT_ARGB8888
//  is a native-endian UInt32 with alpha in the top byte, so no per-pixel
//  conversion or transposition is needed, unlike the 3DS/NDS ports' LCD
//  framebuffers).
//
//  The logical canvas is 320x240 -- exactly the real iPod Classic LCD; SDL's
//  logical presentation scales and letterboxes it to fill the device screen.
//
//  Deliberately imports nothing (no SDL3Swift): the same file compiles on
//  the host for snapshot verification, same as ports/3DS and ports/NDS.
//
//---------------------------------------------------------------------------------
...
struct Canvas {
  ...
  @inline(__always)
  func set(_ x: Int32, _ y: Int32, _ color: UInt32) {
    guard x >= 0, x < width, y >= 0, y < height else { return }
    pixels[Int(y &* width &+ x)] = color
  }
}

// MARK: - Colors (ARGB8888, opaque alpha)

@inline(__always)
func argb32(_ r: Int32, _ g: Int32, _ b: Int32) -> UInt32 {
  0xFF00_0000 | (UInt32(r) << 16) | (UInt32(g) << 8) | UInt32(b)
}

/// Linear interpolation between two 0...255 RGB triples at t/64.
@inline(__always)
func lerpColor(_ a: (Int32, Int32, Int32), _ b: (Int32, Int32, Int32), _ t: Int32) -> UInt32 {
  argb32(
    a.0 &+ ((b.0 &- a.0) &* t) / 64,
    a.1 &+ ((b.1 &- a.1) &* t) / 64,
    a.2 &+ ((b.2 &- a.2) &* t) / 64)
...
let colorBackground = argb32(255, 255, 255)
let colorText = argb32(0, 0, 0)
let colorSelectedText = argb32(255, 255, 255)
let colorSeparator = argb32(115, 115, 120)
let colorDetailText = argb32(115, 115, 120)
let colorScrollTrack = argb32(217, 217, 222)
let colorScrollThumb = argb32(115, 115, 128)
let colorBatteryGreen = argb32(89, 199, 71)
...
func fillRect(_ canvas: Canvas, x: Int32, y: Int32, width: Int32, height: Int32, color: UInt32) {
...
func fillVerticalGradient(
  ...
  while row < height {
    ...
    fillRect(canvas, x: x, y: y &+ row, width: width, height: 1, color: argb32(r, g, b))
    row &+= 1
  }
}
...
/// Alpha-blends `color` over `dst` at 8-bit `coverage` (ARGB8888 channels).
@inline(__always)
func blend32(_ dst: UInt32, _ color: UInt32, _ coverage: Int32) -> UInt32 {
  let dr = Int32((dst >> 16) & 0xFF)
  let dg = Int32((dst >> 8) & 0xFF)
  let db = Int32(dst & 0xFF)
  let sr = Int32((color >> 16) & 0xFF)
  let sg = Int32((color >> 8) & 0xFF)
  let sb = Int32(color & 0xFF)
...
func drawBytes(
  ...
  x: Int32, y: Int32, color: UInt32, maxX: Int32 = Int32.max
) -> Int32 {
  var penX = x
  var i: Int32 = 0
  while i < count {
        ...
        if coverage != 0 {
          ...
          if destX >= 0, destX < canvas.width, destY >= 0, destY < canvas.height {
            ...
            canvas.pixels[index] =
              coverage >= 255
              ? color
              : blend32(canvas.pixels[index], color, coverage)
          }
        }
        gx &+= 1
      }
...
@discardableResult
func drawText(
  _ canvas: Canvas, _ text: StaticString, x: Int32, y: Int32, color: UInt32,
...
@discardableResult
func drawInt(_ canvas: Canvas, _ value: Int32, x: Int32, y: Int32, color: UInt32) -> Int32 {
  var digits = [UInt8]()
  var v = value
  if v < 0 {
    digits.append(45)  // '-'
...
func drawChevron(_ canvas: Canvas, right: Int32, centerY: Int32, color: UInt32) {
  var i: Int32 = 0
  while i < 5 {
...
/// line count (for scroll clamping in AndroidMain.swift).
@discardableResult
...
func compositeSlide(
  present: UnsafeMutablePointer<UInt32>,
  outgoing: UnsafePointer<UInt32>,
  incoming: UnsafePointer<UInt32>,
